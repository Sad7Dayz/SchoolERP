...
namespace SchoolERP.Controllers.Teacher
{
    public class TeacherClassController : Controller
    {
        ...
        {
            List<SubjectMark> subject = new List<SubjectMark>();

            using (SqlConnection con = new SqlConnection(StoreConnection.GetConnection()))
            {
                using (SqlCommand cmd = new SqlCommand("BroadSheet", con))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    if (con.State != System.Data.ConnectionState.Open)
                    ...
                    foreach (DataRow row in dtSessions.Rows)
                    {
                        subject.Add(
                            new SubjectMark
                            {
                                AdmissionNo = Convert.ToInt32(row["AdmissionNo"].ToString()),
                                StudentName = row["FullName"].ToString(),
                                SessionID = row["Session"].ToString(),
            ...
            ViewBag.TotalPages = Math.Ceiling(subject.Count() / 10.0);
            ViewBag.PageNumber = subject;
            subject = subject.Skip((PageNumber - 1) * 10).Take(10).ToList();
            return View(subject);
        }
    }
}